...
def nino_sst_anom(run_case,sst_data,nino,lplot_nino_sst):
...
#	ssta_thresh = np.std(sst_ts)    
    ...
    ''' PLOTTING (TO DO: Move into the figs python collection'''
...
def get_files_tseries(case_name,case_type,var_plot,lats_in,p_levs,years) :
    ...
    allowed_types = ['cam','reanal','lens1','lens2','lense2','cam6_revert','cesm3_dev']
...
# Get the 4 digit format right (e.g., yr = 20 does not parse right automatically.
    fs_yr0 = "{:04}".format(yr0)
    fs_yr1 = "{:04}".format(yr1)
...
#        files_glade = sp.getoutput('ls '+dir_files_stub+'*nc')
    ...
    if case_type in ['cam6_revert','cesm3_dev']: # Complex ensembles structures


#        dir_cam = '/glade/campaign/cgd/amp/rneale/cesm_sims/' # Some output is here
        dir_cam = '/glade/campaign/cgd/amp/amwg/runs/' # Run directories with history files  
    
        if case_type == 'cesm3_dev':
...
# Order files correctly numerically
        iyr0_jan = list(files_glade).index(files_stub+fs_yr0+'-01.nc')
        iyr1_dec = list(files_glade).index(files_stub+fs_yr1+'-12.nc')
...
# Finally trim years.
    ...
    data_files = data_files.sel(lat=slice(lats_in[0],lats_in[1]),time=slice(str(fs_yr0), str(fs_yr1)))
...
def get_files_climo(case_name,case_type,var_plot,lats_in,p_levs,years) :
...
#### Just GRAB the single files for climo, nino and nina.
    ...
    # If derived variable need to get base variable (just, DIV right now)
    if var_plot == 'DIV':
        var_cam = 'OMEGA'
    else:
        var_cam = var_plot
...
def derive_nino_vars(lclimo,var_read,var_plot,p_levs,files_ptr,pfiles_ptr,case_type,var_df,inino_mons,inina_mons,seas_mons):
...
#		if sst_data.time.size != time_in.size : print('SST and VARIABLE sizes DO NOT MATCH - ',sst_data.time.size,' and ',time_in.size) 
    ...
    ## Remember: It is reading in a subset of seaonal months and then nino/nina are a subset of those. 
        ...
        var_in_inseas = var_df.loc[var_read]['vscale']*var_in[imon_seas,:,:,:] # Pull only the months we need
...
#				%time var_in_inseas = var_in_inseas.compute()
    ...
    # Nino/nina anomalies
        ...
        var_in_seas =  files_ptr[var_read].isel(time=0).drop_vars('time')
        var_in_nino =  files_ptr[var_read].isel(time=1).drop_vars('time')
        var_in_nina =  files_ptr[var_read].isel(time=2).drop_vars('time')
        ...
        var_in_seas =  var_df.loc[var_meta]['ovscale']*var_in_seas.loc[max(p_levs):min(p_levs)]
        var_in_nino =  var_df.loc[var_meta]['ovscale']*var_in_nino.loc[max(p_levs):min(p_levs)]
        var_in_nina =  var_df.loc[var_meta]['ovscale']*var_in_nina.loc[max(p_levs):min(p_levs)]
...
def vprof_set_region(ireg,reg_df,varp_in):
    ...
    vweights = np.cos(np.deg2rad(varp_seas.lat))
    vweights.name = "weights"
    
    var_seas = varp_seas.weighted(vweights)
    var_nino = varp_nino.weighted(vweights)
    var_nina = varp_nina.weighted(vweights)
    
    var_seas = var_seas.mean(("lon", "lat"),skipna = True)
    var_nino = var_nino.mean(("lon", "lat"),skipna = True)
    var_nina = var_nina.mean(("lon", "lat"),skipna = True)
    
    #var_seas = var_seas.mean(dim=['lat','lon'],skipna = True)   

    varp_reg = xr.concat([var_seas.compute(), var_nino.compute(), var_nina.compute()], "column")
    
#    varp_reg = (var_seas,var_nino,var_nina) # Put in tuple for looping back in the main code.
    
    
    
    
    return varp_reg,reg_name,reg_w,reg_e,reg_s,reg_n
