...
# Same lat range for now.cesm1
    ...
    match (nino_name):
        ...
        case _: raise ValueError(f'Unknown nino region: {nino_name}')
...
def bin_mean_by_level(
    ...
    """
    Returns DataArray with dims (lev, bin) — mean only.
    Thin wrapper around bin_mean_var_by_level.
    """
    mean_da, _ = bin_mean_var_by_level(field_ts, index_ts, bins, bin_centers, lev_dim, time_dim)
    return mean_da
...
def fig_domains(vname):
    ...
    match vname:
        ...
        case _:
            raise ValueError(f'Variable not recognized in fig_domains: {vname}')

# Return array where gaussian_kde will fit to.
...
# Variable requiring more than 1 timeseries file
    ...
    if files_suff is not None: # CESM1/2
        ...
        if var_2d not in var_derived: # General 3D -> 2D map
            ...
            da_out = get_plev_cam(ds_var, ds_ps, var_plev, var_3dget)
    ...
    if ds_h0 is not None:  # Mostly CESM3 development simulations (muich easier since we already have the Dataset.
        ...
        if var_2d not in var_derived:  # General 3D -> 2D map
            ...
            da_out = get_plev_cam(ds_h0,ds_h0,var_plev,var_3dget)            
    
        else: # Special cases
    
            if var_plev == 'DIV':
    
                da_uplev = get_plev_cam(ds_h0,ds_h0,var_plev,'U') 
                da_vplev = get_plev_cam(ds_h0,ds_h0,var_plev,'V') 
...
#            da_axis = xr.open_mfdataset(files_ls,parallel=True, combine="by_coords",data_vars="minimal", coords="minimal")[var_axis]
    ...
    #                    print(yr_str)
                    ...
                    files_hist = glob.glob(file_hist)
                    files_hist.sort()
