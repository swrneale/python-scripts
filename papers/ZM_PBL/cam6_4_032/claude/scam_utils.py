...
def moving_average(x, w):
    # Gaussian smooth: sigma = w/3 gives an effective smoothing width
    # comparable to a boxcar of width w, but without the blocky step edges.
    sigma = w / 3.0
    return gaussian_filter1d(np.asarray(x, dtype=float), sigma=sigma)
