...
# A bit tricky but see how many black lines there are and change the symbol if it is not the first obs. case plotted
    ...
    if nobs_lines: 
        params_lat.update({'marker': omarker[nobs_lines]})
        params_lat.update({'linestyle': olstyle[nobs_lines]})
...
#        axr.set_ylim(-0.2,0.2)   # fixed range on right-hand axis
        ...
        zm_line, = ax.plot(zmp['lat'], zmp, label=cname, markersize=8, markevery=3, **params_lat)
...
### 2D VARS ###RMSD
        case 'PRECT' | 'PRECC' | 'PRECL':    
            ...
            aclevs = np.array([-12, -10., -8, -6, -4, -3, -2, -1, 0, 1, 2, 3, 4, 6, 8, 10, 12])*1.0
            cmap = 'terrain_r'
            acmap = 'PRGn'
...
#    var_in = var_in.squeeze 
 
    ndims = len(var_in.dims)
...
# Is LANDFRAC in the current dataset (alsmot always for CAM output/climo files.)
    ...
    else: # Use a generic mask and interplolate.
